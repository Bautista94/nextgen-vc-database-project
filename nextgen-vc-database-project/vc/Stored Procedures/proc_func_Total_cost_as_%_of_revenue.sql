/* ================================================================================================================================================
   Procedure Name : [vc].[proc_func_Total_cost_as_%_of_revenue]
   Purpose : Show the 'Total Cost as a percentage of Revenue' benchmark for every function in the gap to benchmark views.
   Last Modified: 21/02/2023
   Date Modified:
   Inputs :
		@input_year: ID for year
		@designation_id: ID for designation
		@benchmark_metric: ID for the 'Total Cost as a percentage of Revenue' benchmark
   
   Outputs:
	   A result set containing the columns for the 'Total Cost as a percentage of Revenue' benchmark drop down option it the gap to benchmark views.
...
CREATE   PROCEDURE [vc].[proc_func_Total_cost_as_%_of_revenue](
    @input_year int,
    @designation_id int,
...
SELECT
	...
	--MEDIAN
	--,SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9))  as median_customer_metric
	--,GREATEST(((SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9))) - median)/ (SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9))),0) as median_savings_multiplier
	--,GREATEST((((SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9))) - median)/ (SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9)))) * SUM([labor_amount]),0) as median_implied_savings

		--COST
	,SUM(designation_revenue) * median as median_potential_amount
	,SUM(designation_revenue) * median / SUM(COALESCE([labor_amount],0)+COALESCE([non_labor_amount],0)) as median_potential_percent_baseline
	,SUM(designation_revenue) * median / SUM(COALESCE([labor_amount],0)+COALESCE([non_labor_amount],0)) * SUM(employee_count) as median_potential_fte
		--GAP
	,CASE WHEN [median] is null then null else GREATEST(SUM(COALESCE([labor_amount],0)+COALESCE([non_labor_amount],0)) - (SUM(designation_revenue) * median),0) END as median_opportunity_amount
	,CASE WHEN [median] is null then null else GREATEST((SUM(COALESCE([labor_amount],0)+COALESCE([non_labor_amount],0)) - (SUM(designation_revenue) * median)) / SUM(COALESCE([labor_amount],0)+COALESCE([non_labor_amount],0)),0) END as median_opportunity_percent_baseline
	,CASE WHEN [median] is null then null else GREATEST(SUM([employee_count])*(SUM(COALESCE([labor_amount],0)+COALESCE([non_labor_amount],0)) - (SUM(designation_revenue) * median)) / SUM(COALESCE([labor_amount],0)+COALESCE([non_labor_amount],0)),0) END as median_opportunity_fte


	--TOP QUARTILE
		--COST
	,SUM(designation_revenue) * top_quartile as top_quartile_potential_amount
	,SUM(designation_revenue) * top_quartile / SUM(COALESCE([labor_amount],0)+COALESCE([non_labor_amount],0)) as top_quartile_potential_percent_baseline
	,SUM(designation_revenue) * top_quartile / SUM(COALESCE([labor_amount],0)+COALESCE([non_labor_amount],0)) * SUM(employee_count) as top_quartile_potential_fte
		--GAP
	,CASE WHEN [top_quartile] is null then null else GREATEST(SUM(COALESCE([labor_amount],0)+COALESCE([non_labor_amount],0)) - (SUM(designation_revenue) * top_quartile),0) END as top_quartile_opportunity_amount
	,CASE WHEN [top_quartile] is null then null else GREATEST((SUM(COALESCE([labor_amount],0)+COALESCE([non_labor_amount],0)) - (SUM(designation_revenue) * top_quartile)) / SUM(COALESCE([labor_amount],0)+COALESCE([non_labor_amount],0)),0) END as top_quartile_opportunity_percent_baseline
	,CASE WHEN [top_quartile] is null then null else GREATEST(SUM([employee_count])*(SUM(COALESCE([labor_amount],0)+COALESCE([non_labor_amount],0)) - (SUM(designation_revenue) * top_quartile)) / SUM(COALESCE([labor_amount],0)+COALESCE([non_labor_amount],0)),0) END as top_quartile_opportunity_fte
