/* ================================================================================================================================================
   Procedure Name : [vc].[proc_func_FTEs_per_$1B_revenue]
   Purpose : Show the 'FTEs per 1 billion of revenue' benchmark for every function in the gap to benchmark views.
   ...
   Inputs :
		...
		@benchmark_metric: ID for the 'FTEs per 1 billion of revenue' benchmark
   
   Outputs:
	   A result set containing the columns for the 'FTEs per 1 billion of revenue' benchmark drop down option it the gap to benchmark views.
...
CREATE   PROCEDURE [vc].[proc_func_FTEs_per_$1B_revenue](
    @input_year int,
    @designation_id int,
...
SELECT
	...
	--,GREATEST((((SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9))) - median)/ (SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9)))) * SUM([labor_amount]),0) as median_implied_savings
		...
		--COST
	,SUM([labor_amount]) * median / (SUM([employee_count]) / (SUM(designation_revenue)/POWER(10, 9))) as median_potential_amount
	...
	,SUM([employee_count]) * median / (SUM([employee_count]) / (SUM(designation_revenue)/POWER(10, 9))) as median_potential_fte
		--GAP
	,CASE WHEN [median] is null then null else GREATEST((((SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9))) - median)/ (SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9)))) * SUM([labor_amount]),0) END as median_opportunity_amount
	,CASE WHEN [median] is null then null else GREATEST(((SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9))) - median)/ (SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9))),0) END as median_opportunity_percent_baseline
	,CASE WHEN [median] is null then null else GREATEST(SUM([employee_count])*((SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9))) - median)/ (SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9))),0) END as median_opportunity_fte


	--TOP QUARTILE
		--COST
	,SUM([labor_amount]) * top_quartile / (SUM([employee_count]) / (SUM(designation_revenue)/POWER(10, 9))) as top_quartile_potential_amount
	...
	,SUM([employee_count]) * top_quartile / (SUM([employee_count]) / (SUM(designation_revenue)/POWER(10, 9))) as top_quartile_potential_fte
		--GAP
	,CASE WHEN [top_quartile] is null then null else GREATEST((((SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9))) - top_quartile)/ (SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9)))) * SUM([labor_amount]),0) END as top_quartile_opportunity_amount
	,CASE WHEN [top_quartile] is null then null else GREATEST(((SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9))) - top_quartile)/ (SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9))),0) END as top_quartile_opportunity_percent_baseline
	,CASE WHEN [top_quartile] is null then null else GREATEST(SUM([employee_count])*((SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9))) - top_quartile)/ (SUM([employee_count])/(SUM(designation_revenue)/POWER(10, 9))),0) END as top_quartile_opportunity_fte
