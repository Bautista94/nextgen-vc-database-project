/* ================================================================================================================================================
   Procedure Name : [vc].[proc_func_Personnel_cost_as_%_of_revenue]
   Purpose : Show the 'Personnel cost as percentage of revenue' benchmark for every function in the gap to benchmark views.
   ...
   Inputs :
		...
		@benchmark_metric: ID for the 'Personnel cost as percentage of revenue' benchmark
   
   Outputs:
	   A result set containing the columns for the 'Personnel cost as percentage of revenue' benchmark drop down option it the gap to benchmark views.
...
CREATE   PROCEDURE [vc].[proc_func_Personnel_cost_as_%_of_revenue](
    @input_year int,
    @designation_id int,
...
SELECT
	...
	,SUM(designation_revenue) * median / SUM(labor_amount) as median_potential_percent_baseline
	,SUM(designation_revenue) * median / SUM(labor_amount) * SUM(employee_count) as median_potential_fte
		--GAP
	,CASE WHEN [median] is null then null else GREATEST(SUM([labor_amount]) - (SUM(designation_revenue) * median),0) END as median_opportunity_amount
	,CASE WHEN [median] is null then null else GREATEST((SUM([labor_amount]) - (SUM(designation_revenue) * median)) / SUM([labor_amount]),0) END as median_opportunity_percent_baseline
	,CASE WHEN [median] is null then null else (SUM([labor_amount]) - (SUM(designation_revenue) * median)) / SUM([labor_amount]) * SUM(employee_count) END as median_opportunity_fte
	--,SUM([labor_amount])/SUM(designation_revenue) as median_customer_metric
	--,GREATEST(((SUM([labor_amount])/SUM(designation_revenue)) - median)/ (SUM([labor_amount])/SUM(designation_revenue)),0) as median_savings_multiplier
	--,GREATEST((((SUM([labor_amount])/SUM(designation_revenue)) - median)/ (SUM([labor_amount])/SUM(designation_revenue))) * SUM([labor_amount]),0) as median_implied_savings

	--TOP QUARTILE
		--COST
	,SUM(designation_revenue) * top_quartile as top_quartile_potential_amount
	,SUM(designation_revenue) * top_quartile / SUM(labor_amount) as top_quartile_potential_percent_baseline
	,SUM(designation_revenue) * top_quartile / SUM(labor_amount) * SUM(employee_count) as top_quartile_potential_fte
		--GAP
	,CASE WHEN [top_quartile] is null then null else GREATEST(SUM([labor_amount]) - (SUM(designation_revenue) * top_quartile),0) END as top_quartile_opportunity_amount
	,CASE WHEN [top_quartile] is null then null else GREATEST((SUM([labor_amount]) - (SUM(designation_revenue) * top_quartile)) / SUM([labor_amount]),0) END as top_quartile_opportunity_percent_baseline
	,CASE WHEN [top_quartile] is null then null else (SUM([labor_amount]) - (SUM(designation_revenue) * top_quartile)) / SUM([labor_amount]) * SUM(employee_count) END as top_quartile_opportunity_fte
	--,SUM([labor_amount])/SUM(designation_revenue) as top_quartile_customer_metric
	--,GREATEST(((SUM([labor_amount])/SUM(designation_revenue)) - top_quartile)/ (SUM([labor_amount])/SUM(designation_revenue)),0) as top_quartile_savings_multiplier
	--,GREATEST((((SUM([labor_amount])/SUM(designation_revenue)) - top_quartile)/ (SUM([labor_amount])/SUM(designation_revenue))) * SUM([labor_amount]),0) as top_quartile_implied_savings
